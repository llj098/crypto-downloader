...
class Binance(object):
    ...
    # Max trades retrievable by Binance on one request
    LIMIT = 1000
    ...
    def get_trades(cls, base_trade_number, pair):
        ...
        url = '{}{}?symbol={}&fromId={}&limit={}'.format(cls.BASE_URL, '/api/v1/aggTrades', pair, base_trade_number, cls.LIMIT)
        print(url)
        ...
        #not trades[0]['a'] == base_trade_number + cls.LIMIT - 1
        ...
        return ([dict(trade_id=trade['a'], size=float(trade['q']),
                      side="sell" if trade['m'] else 'buy', price=float(trade['p']),
                      time=trade['T']) for trade in trades], last)
    ...
    def write_trades_from(cls, base_trade_id, file_path, pair):
        ...
        while not is_last_trade:
            ...
            try:
                ...
                print ("OOps: Something Else", sys.exc_info()[0])
                raise
                time.sleep(3)
                sys.stdout.flush()
